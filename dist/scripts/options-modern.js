...
class ModernDashboard {
    ...
    showRuleForm(rule, index = -1) {
        ...
        if (rule) {
            ...
            if (title)
                title.textContent = 'Edit Download Rule';
        }
        else {
            this.clearForm();
            ...
            const title = document.getElementById('form-title');
            if (title)
                title.textContent = 'New Download Rule';
        }
        emptyState?.style.setProperty('display', 'none');
    ...
    clearForm() {
        ...
        const conflictSelect = document.getElementById('rule-conflict');
        if (conflictSelect)
            conflictSelect.value = 'uniquify';
    }
    async saveRule() {
        const getValue = (id) => {
            const element = document.getElementById(id);
        ...
        const ruleData = {
            ...
            pattern: pattern,
            'conflict-action': getValue('rule-conflict'),
            enabled: true
        };
        try {
    ...
    generateRuleDescription(rule) {
        const matchers = [];
        if (rule.url)
            matchers.push(`URL: ${this.truncate(rule.url, 25)}`);
        if (rule.mime)
            matchers.push(`Type: ${this.truncate(rule.mime, 25)}`);
        if (rule.filename)
            matchers.push(`File: ${this.truncate(rule.filename, 25)}`);
    ...
    generateRuleBadges(rule) {
        const badges = [];
        if (rule.url)
            badges.push('<span class="badge badge-url">URL</span>');
        if (rule.mime)
            badges.push('<span class="badge badge-mime">MIME</span>');
        if (rule.filename)
            badges.push('<span class="badge badge-filename">FILE</span>');
        if (rule.referrer)
            badges.push('<span class="badge badge-referrer">REF</span>');
        if (rule.tabUrl)
            badges.push('<span class="badge badge-taburl">TAB</span>');
        if (!rule.enabled)
    ...
    setupDragAndDrop() {
        ...
        list.addEventListener('drop', async (e) => {
            ...
            if (target && draggedElement && target !== draggedElement) {
                ...
                setTimeout(() => {
                    const newItem = list.querySelector(`[data-index="${targetIndex}"]`);
                    if (newItem) {
    ...
    async checkPermissions() {
        try {
            ...
            if (hasTabUrlRules && !hasTabsPermission) {
                this.showAlert('Some rules use Tab URL matching but tabs permission is not granted. <button onclick="dashboard.requestTabsPermission()" class="btn btn-sm btn-primary">Grant Permission</button>', 'warning');
            }
        }
        catch (error) {
    ...
    showCustomDialog(content, title) {
